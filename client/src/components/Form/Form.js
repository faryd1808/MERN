  ...
  if (!user?.result?.name) {
    return (
      <Paper className={classes.paper} elevation={6}>
        ...
        Inicie sesión para crear sus propios recuerdos y darle me gusta a los recuerdos de otros.
        </Typography>
      </Paper>
    );
  }
  ...
  return (
    <Paper className={classes.paper} elevation={6}>
      <form autoComplete="off" noValidate className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
        <Typography variant="h6">{currentId ? `Editing "${post?.title}"` : 'Crea un Recuerdo'}</Typography>
        ...
        <TextField name="message" variant="outlined" label="Descripcion" fullWidth multiline rows={4} value={postData.message} onChange={(e) => setPostData({ ...postData, message: e.target.value })} />
